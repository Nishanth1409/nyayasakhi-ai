/**
 * Vercel serverless proxy for AI chat (same logic as supabase/functions/nyaya-chat).
 * Set LOVABLE_API_KEY in Vercel → Environment Variables (Production + Preview).
 */
export const config = {
  runtime: "edge",
};
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!apiKey) {
      return new Response(
        JSON.stringify({
          error: "LOVABLE_API_KEY is not set. Add it in Vercel → Settings → Environment Variables, then redeploy.",
        }),
    ...
    console.error("nyaya-chat api error:", e);
    return new Response(
